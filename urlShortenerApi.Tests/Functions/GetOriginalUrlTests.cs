...
public class GetOriginalUrlTests
{
    ...
    public void Initialize(Mock<ISession>? session = null)
    {
        ...
        this.function = new GetOriginalUrl(mockLoggerFactory.Object, databaseSessionFactoryMock.Object);
    }

    private FunctionContext? context;
    private GetOriginalUrl? function;
    ...
    public async Task UrlDoesNotExists(string shortUrl)
    {
        ...
        await ExecuteTest(shortUrl, HttpStatusCode.NotFound, async (result) =>
        {
            var bodyResponse = await Helper.GetBodyResponse(result.Body);
            Assert.IsType<string>(bodyResponse);
            Assert.NotNull(bodyResponse);
            Assert.Equal(bodyResponse, "Url does not exists");
        });
    }
    ...
    public async Task UrlExists(string shortUrl)
    {
        ...
        Initialize(sessionMock);
        await ExecuteTest(shortUrl, HttpStatusCode.OK, async (result) =>
        {
            var bodyResponse = await Helper.GetBodyResponse(result.Body);
            Assert.IsType<string>(bodyResponse);
            Assert.NotNull(bodyResponse);
            Assert.Equal(JsonSerializer.Serialize(new { Url = expectedUrl }), bodyResponse);
        });
    }
